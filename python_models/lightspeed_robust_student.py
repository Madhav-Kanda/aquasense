...
for eps_i in range(len(y)):
    ...
    while True:
        ...
        robust_t_nu = torch.arange(ma_eps, 10 + ma_eps, step=(10 - ma_eps)/splits, device=aqua_device)
        robust_t_nu = torch.reshape(robust_t_nu, [1, 1, 1, -1])
        densityCube_p = densityCube_p + torch.sum(tdist.StudentT(robust_t_nu, loc=beta[0], scale=sigma).log_prob(y).to(aqua_device), 0, keepdim=True)
        ...
        posterior_robust_t_nu = expDensityCube_p.sum([1, 2, 0]) / z_expDensityCube
        sigma = sigma.flatten()
    ...
    #print((robust_t_nu.flatten() * posterior_robust_t_nu).sum())
