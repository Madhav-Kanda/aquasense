...
for eps_i in range(len(y)):
    ...
    while True:
        ...
        for i in range(1, N + 1):
            ...
            robust_weight[i-1] = torch.reshape(robust_weight[i-1], [1, 1, 1, -1])
            densityCube_p = densityCube_p + +(-torch.log(sigma)- 0.9189385332046727 - 0.5 * torch.pow((beta[0] - (y.view(-1)[i-1])) / (sigma), 2))*robust_weight[i-1]
