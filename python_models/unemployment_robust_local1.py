...
for eps_i in range(len(y)):
    ...
    while True:
        ...
        robust_local_beta1 = [None] * 40
        for i in range(1, N + 1):
            robust_local_beta1[i-1] = torch.arange(ma_eps, 14 + ma_eps, step=(14 - ma_eps)/splits, device=aqua_device)
            robust_local_beta1[i-1] = torch.reshape(robust_local_beta1[i-1], [1, 1, 1, 1, -1])
            densityCube_p = densityCube_p + (-torch.log(torch.tensor(1))- 0.9189385332046727 - 0.5 * torch.pow((beta[0]+beta[1]*y_lag.view(-1)[i-1] - (robust_local_beta1[i-1])) / (torch.tensor(1)), 2))
            densityCube_p = densityCube_p + (-torch.log(sigma)- 0.9189385332046727 - 0.5 * torch.pow((robust_local_beta1[i-1] - (y.view(-1)[i-1])) / (sigma), 2))
