...
y_lag = torch.reshape(y_lag, [-1, 1, 1, 1, 1])
...
for eps_i in range(len(y)):
    ...
    while True:
        if repeat:
            ...
            splits = int(sys.argv[1])
        densityCube_p = torch.zeros(1, 101, 101, 101, 101, device=aqua_device)
        beta = [None] * 2
        ...
        beta[1] = torch.reshape(beta[1], [1, 1, -1, 1, 1])
        ...
        robust_t_nu = torch.arange(ma_eps, 10 + 2 * ma_eps, step=(10)/splits, device=aqua_device)
        robust_t_nu = torch.reshape(robust_t_nu, [1, 1, 1, 1, -1])
        for observe_i in range(1,2): # N + 1):
            densityCube_p.add_(+tdist.StudentT(robust_t_nu, loc=beta[0]+(beta[1])*(y_lag.view(-1)[observe_i-1]), scale=sigma).log_prob(y.view(-1)[observe_i-1]).to(aqua_device))
            #print(using("iter" + str(observe_i)))
            print(get_gpu_memory())

        expDensityCube_p = torch.exp(densityCube_p - torch.max(densityCube_p))
        z_expDensityCube = torch.sum(expDensityCube_p)
        posterior_sigma = expDensityCube_p.sum([1, 2, 0, 4]) / z_expDensityCube
        posterior_beta[0] = expDensityCube_p.sum([0, 2, 3, 4]) / z_expDensityCube
        posterior_beta[1] = expDensityCube_p.sum([1, 0, 3, 4]) / z_expDensityCube
        posterior_robust_t_nu = expDensityCube_p.sum([1, 2, 3, 0]) / z_expDensityCube
        sigma = sigma.flatten()
        ...
        robust_t_nu = robust_t_nu.flatten()
        posterior_robust_t_nu = posterior_robust_t_nu.flatten()
        ...
        del sigma
        del robust_t_nu
        del expDensityCube_p
        del densityCube_p
        del posterior_beta
        del posterior_sigma
        del posterior_robust_t_nu
        del lowProb_beta
        del all_gt_beta
        del lowProb_sigma
        del all_gt_sigma
    ...
    break
    #print((robust_t_nu.flatten() * posterior_robust_t_nu).sum().item())
