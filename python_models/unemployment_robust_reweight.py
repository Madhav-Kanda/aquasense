...
for eps_i in range(len(y)):
    ...
    while True:
        ...
        robust_weight = [None] * 40
        for i in range(1, N + 1):
            robust_weight[i-1] = torch.arange(ma_eps, 1 + ma_eps, step=(1 - ma_eps)/splits, device=aqua_device)
            robust_weight[i-1] = torch.reshape(robust_weight[i-1], [1, 1, 1, 1, -1])
            densityCube_p = densityCube_p + +(-torch.log(sigma)- 0.9189385332046727 - 0.5 * torch.pow((beta[0]+beta[1]*y_lag.view(-1)[i-1] - (y.view(-1)[i-1])) / (sigma), 2))*robust_weight[i-1]
    ...
    #print(sigma.flatten())
    #print(posterior_sigma.flatten())
    #print(beta[1].flatten())
    #print(posterior_beta[1].flatten())
