...
for eps_i in range(len(y)):
    ...
    while True:
        ...
        for i in range(1, N + 1):
            robust_local_tau[i-1] = torch.arange(ma_eps, 10 + ma_eps, step=(10 - ma_eps)/splits, device=aqua_device)
            robust_local_tau[i-1] = torch.reshape(robust_local_tau[i-1], [1, 1, 1, 1, -1])
            densityCube_p = densityCube_p + tdist.Gamma(2.5, 2.5).log_prob(robust_local_tau[i-1]).to(aqua_device)
            densityCube_p = densityCube_p + (-torch.log(sigma*(1 / (torch.sqrt(robust_local_tau[i-1]))))- 0.9189385332046727 - 0.5 * torch.pow((beta[0]+beta[1]*y_lag.view(-1)[i-1] - (y.view(-1)[i-1])) / (sigma*(1 / (torch.sqrt(robust_local_tau[i-1])))), 2))
    ...
    #print(posterior_beta[0].flatten())
