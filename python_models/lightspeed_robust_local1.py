...
for eps_i in range(len(y)):
    ...
    while True:
        ...
        for i in range(1, N + 1):
            robust_local_beta1[i-1] = torch.arange(16 + ma_eps, 38 + ma_eps, step=(35 - ma_eps - 20)/splits, device=aqua_device)
            robust_local_beta1[i-1] = torch.reshape(robust_local_beta1[i-1], [1, 1, 1, -1])
            densityCube_p = densityCube_p + (-torch.log(torch.tensor(1))- 0.9189385332046727 - 0.5 * torch.pow((beta[0] - (robust_local_beta1[i-1])) / (torch.tensor(1)), 2))
            densityCube_p = densityCube_p + (-torch.log(sigma)- 0.9189385332046727 - 0.5 * torch.pow((robust_local_beta1[0] - (y.view(-1)[i-1])) / (sigma), 2))
            densityCube_p = torch.logsumexp(densityCube_p, 3, keepdim=True)
    
        expDensityCube_p = torch.exp(densityCube_p - torch.max(densityCube_p))
        z_expDensityCube = torch.sum(expDensityCube_p)
        posterior_sigma = expDensityCube_p.sum([1, 0, 3]) / z_expDensityCube
